...
try {
    ...
    $check = $pdo->prepare("
        SELECT COUNT(*) FROM status WHERE description = :description AND userid = :userid
    ");
    $check->execute([
    ...
    if ($check->fetchColumn() > 0) {
        $pdo->rollBack();
        header('Location: Stati.php?exists=1', true, 303);
        exit;
    }
    ...
    header('Location: Stati.php?success=1', true, 303);
    exit;
} catch (Throwable $e) {
    ...
    error_log('Stati-Insert-Fehler: ' . $e->getMessage());
