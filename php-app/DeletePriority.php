...
try {
   $stmt = $pdo->prepare("DELETE FROM priority WHERE PriorityId = :PriorityId");
   $stmt->bindValue(':PriorityId', $id, PDO::PARAM_INT);
   $stmt->execute();
  ...
  echo "Helpdesk Priorität - Position " . htmlspecialchars((string) $id, ENT_QUOTES, 'UTF-8') . " wurde gelöscht!";
